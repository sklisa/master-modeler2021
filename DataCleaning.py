    ...
    with open(file, 'r') as f:
        ...
        try:
            ...
            if 'message' in dct and dct['message'] is not None:
                ...
                pr.fully_preprocess()
                new_dct['cleaned_message'] = pr.text.split()
                # print('cleaned text')
            if 'message_tags' in dct:
                message_tags = dct['message_tags']
                new_dct['message_tags'] = [i['name'].replace('#','') for i in message_tags]
            if 'attachments' in dct:
                ...
                for i, attachment in enumerate(attachments):
                    ...
                    image = None
                    if 'media' in attachment and 'image' in attachment['media']['image']:
